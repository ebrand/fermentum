...
const BreweryOperationsDrawer = ({ isOpen, onClose }) => {
  const sessionContext = useSession()
  const { user, userTenants, currentTenant, setCurrentTenant } = sessionContext
  ...
  const planInfo = useMemo(() => {
    ...
    const planName = firstTenant.planName || 'Starter'
    const breweryLimit = firstTenant.breweryLimit || 1
    const userLimit = firstTenant.userLimit || 3
  ...
  const handleInviteUser = async () => {
    ...
    try {
      // Find the brewery for this tenant - for now, use the first brewery
      const brewery = userTenants.find(t => t.tenantId === currentTenant.tenantId)?.breweries?.[0]
      if (!brewery) {
        showError('No brewery found for this tenant')
        return
      }

      const response = await invitationsAPI.createInvitation({
        email: inviteEmail,
        breweryId: brewery.breweryId || currentTenant.tenantId, // Use tenantId as fallback for now
        role: inviteRole
      })

      if (response.data.success) {
        // Show success notification with demo message
        showSuccess(response.data.message || 'Invitation sent successfully! DEMO: Email notification would be sent in production.')
        ...
        // Refresh users and invitations lists
        loadTenantUsers()
        loadTenantInvitations()
      } else {
        showError(response.data.message || 'Failed to send invitation')
      }
    } catch (error) {
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900 font-['Inter','-apple-system','system-ui','sans-serif']">
                  Invite Team Member
                </h3>
                <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent font-['Menlo','Monaco','monospace']"
                ...
                <div className="text-sm text-gray-500 font-['Menlo','Monaco','monospace']">
              ...
              <div className="flex items-center justify-end space-x-3 mt-6">
                <button
                  onClick={() => setShowInviteModal(false)}
                  className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors duration-150 font-['Menlo','Monaco','monospace']"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium text-white bg-purple-600 rounded-md hover:bg-purple-700 disabled:bg-gray-400 transition-colors duration-150 font-['Menlo','Monaco','monospace'] disabled:cursor-not-allowed"
                >
                  {inviteLoading ? (
                    <div className="flex items-center">
