-- Migration 019 Fixed: Update database schema to match ERD exactly
...
-- Step 2: Create Tenant_PaymentMethod junction table if not exists
...
-- Save existing data that has valid TenantId references
CREATE TEMP TABLE brewery_backup AS
SELECT b."TenantId", b."Name", b."Created", b."CreatedBy", b."Updated", b."UpdatedBy"
FROM "Brewery" b
WHERE EXISTS (SELECT 1 FROM "Tenant" t WHERE t."TenantId" = b."TenantId");
...
-- Restore valid data to Brewery table (new UUIDs will be generated)
...
-- Save existing employee data that has valid TenantId references
CREATE TEMP TABLE employee_backup AS
SELECT e."TenantId", e."FirstName", e."LastName", e."IsActive", e."Created", e."CreatedBy", e."Updated", e."UpdatedBy", e."UserId"
FROM "Employee" e
WHERE EXISTS (SELECT 1 FROM "Tenant" t WHERE t."TenantId" = e."TenantId");
...
-- Restore employee data (without BreweryId references for now - they'll need to be reassigned)
